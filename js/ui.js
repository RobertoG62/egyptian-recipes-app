...
const UI = (() => {
    // Egyptian cuisine categories with icons
    const CATEGORY_ICONS = {
        // Hebrew
        'הכל': 'fa-utensils',
        'פול וטעמיה': 'fa-bowl-food',
        'מלוכיה ותבשילים': 'fa-leaf',
        'אורז ומאפים': 'fa-bowl-rice',
        'דגים וסלטים': 'fa-fish',
        'קינוחים ומשקאות': 'fa-mug-hot',
        // English
        'All': 'fa-utensils',
        'Ful and Taamia': 'fa-bowl-food',
        'Mulukhiyah & Stews': 'fa-leaf',
        'Rice & Breads': 'fa-bowl-rice',
        'Fish & Salads': 'fa-fish',
        'Sweets & Drinks': 'fa-mug-hot',
    };
    ...
    function renderCards(recipes) {
        ...
        grid.innerHTML = recipes.map(recipe => `
            <article class="recipe-card card-lift" data-id="${recipe.id}">
                ...
                <div class="p-4">
                    <h3 class="font-bold text-lg mb-0.5 text-eg-charcoal">${recipe.title}</h3>
                    ${recipe.originalName ? `<p class="text-eg-primary-dark text-xs italic mb-2 font-playfair">${recipe.originalName}</p>` : ''}
                    <p class="text-eg-text-secondary text-sm line-clamp-2 mb-3">${recipe.description}</p>
                    <div class="flex items-center justify-between text-xs text-eg-text-secondary">
            ...
            <div class="view-fade-in">
                <div class="recipe-hero" style="background-image: url('${recipe.image}')">
                    <div class="relative z-10 w-full p-6 md:p-10 max-w-4xl mx-auto">
                        <span class="inline-block px-3 py-1 rounded-full text-xs font-medium bg-eg-primary text-white mb-3">
                        ...
                        ${recipe.originalName ? `<p class="text-eg-primary-light text-base md:text-lg italic font-playfair mt-1">${recipe.originalName}</p>` : ''}
                ...
                <div class="max-w-4xl mx-auto px-4 -mt-8 relative z-10">
                    <div class="glass rounded-xl p-4 flex justify-center gap-0 mb-8">
                        <div class="meta-item">
                            <i class="far fa-clock text-eg-primary"></i>
                            <span class="font-bold text-eg-charcoal">${recipe.prepTime} ${i18n.t('detail.minutes')}</span>
                            <span class="text-xs text-eg-text-secondary">${i18n.t('detail.prepTime')}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas fa-fire text-eg-primary"></i>
                            <span class="font-bold text-eg-charcoal">${recipe.cookTime} ${i18n.t('detail.minutes')}</span>
                            <span class="text-xs text-eg-text-secondary">${i18n.t('detail.cookTime')}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas fa-users text-eg-primary"></i>
                            <span class="font-bold text-eg-charcoal">${recipe.servings}</span>
                            <span class="text-xs text-eg-text-secondary">${i18n.t('detail.servings')}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas fa-gauge text-eg-primary"></i>
                            <span class="font-bold text-eg-charcoal">${recipe.difficulty}</span>
                            <span class="text-xs text-eg-text-secondary">${i18n.t('detail.difficulty')}</span>
                        </div>
                        <div class="meta-item">
                            <i class="fas ${getKosherBadge(recipe.kosherType).icon} text-eg-primary"></i>
                            <span class="font-bold text-eg-charcoal">${recipe.kosherType}</span>
                            <span class="text-xs text-eg-text-secondary">${i18n.t('detail.kosher')}</span>
                        </div>
                    </div>

                    <div class="glass gold-border-right rounded-xl p-6 mb-8">
                        <h2 class="font-bold text-xl mb-4 text-eg-charcoal flex items-center gap-2">
                            <i class="fas fa-list text-eg-primary"></i>
                            ${i18n.t('detail.ingredients')}
                            <span class="text-sm font-normal text-eg-text-secondary">(${recipe.ingredients.length})</span>
                        </h2>
                        <div>
                            ${recipe.ingredients.map(ing => `
                                <div class="ingredient-row">
                                    <span class="text-eg-primary"><i class="fas fa-check text-xs"></i></span>
                    ...
                    <div class="mb-8">
                        <h2 class="font-bold text-xl mb-6 text-eg-charcoal flex items-center gap-2">
                            <i class="fas fa-list-ol text-eg-primary"></i>
                        ...
                        <div class="space-y-4">
                            ${recipe.instructions.map((step, i) => `
                                <div class="flex gap-4 items-start">
                                    ...
                                    <p class="text-eg-charcoal pt-1 leading-relaxed">${mdBold(step)}</p>
                                </div>
                            `).join('')}
                        </div>
                    </div>
